    ...
    if (selector) {
        if (typeof selector === 'string') {
            ...
            if (html.indexOf('<') >= 0 && html.indexOf('>') >= 0) {
                ...
                if (!context && selector[0] === '#' && !selector.match(/[ .<>:~]/)) { els = [document.getElementById(selector.trim().split('#')[1])] } else { els = (context || document).querySelectorAll(selector.trim()) }
                for (i = 0; i < els.length; i += 1) { if (els[i]) arr.push(els[i]) }
            }
        } else if (selector.nodeType || selector === window || selector === document) { arr.push(selector) } else if (selector.length > 0 && selector[0].nodeType) { for (i = 0; i < selector.length; i += 1) { arr.push(selector[i]) } }
    }
    return new Dom7(arr)
}
$.fn = Dom7.prototype;
...
function addClass(className) {
    ...
    for (let i = 0; i < classes.length; i += 1) { for (let j = 0; j < this.length; j += 1) { if (typeof this[j] !== 'undefined' && typeof this[j].classList !== 'undefined') this[j].classList.add(classes[i]) } }
    return this
}

function removeClass(className) {
    const classes = className.split(' ');
    for (let i = 0; i < classes.length; i += 1) { for (let j = 0; j < this.length; j += 1) { if (typeof this[j] !== 'undefined' && typeof this[j].classList !== 'undefined') this[j].classList.remove(classes[i]) } }
    return this
}
...
function toggleClass(className) {
    const classes = className.split(' ');
    for (let i = 0; i < classes.length; i += 1) { for (let j = 0; j < this.length; j += 1) { if (typeof this[j] !== 'undefined' && typeof this[j].classList !== 'undefined') this[j].classList.toggle(classes[i]) } }
    return this
}
...
function attr(attrs, value) {
    ...
    for (let i = 0; i < this.length; i += 1) {
        if (arguments.length === 2) { this[i].setAttribute(attrs, value) } else {
...
function prop(props, value) {
    if (arguments.length === 1 && typeof props === 'string') { if (this[0]) return this[0][props] } else {
        for (let i = 0; i < this.length; i += 1) { if (arguments.length === 2) { this[i][props] = value } else { for (const propName in props) { this[i][propName] = props[propName] } } }
        return this
    }
}
...
function dataset() {
    ...
    const dataset = {};
    if (el.dataset) { for (const dataKey in el.dataset) { dataset[dataKey] = el.dataset[dataKey] } } else { for (let i = 0; i < el.attributes.length; i += 1) { const attr = el.attributes[i]; if (attr.name.indexOf('data-') >= 0) { dataset[toCamelCase(attr.name.split('data-')[1])] = attr.value } } }
    for (const key in dataset) {
        if (dataset[key] === 'false') dataset[key] = !1;
        else if (dataset[key] === 'true') dataset[key] = !0;
...
function trigger(...args) {
    ...
    for (let i = 0; i < events.length; i += 1) {
        ...
        for (let j = 0; j < this.length; j += 1) {
            const el = this[j];
            let evt;
            try { evt = new window.CustomEvent(event, { detail: eventData, bubbles: !0, cancelable: !0, }) } catch (e) {
...
function offset() {
    if (this.length > 0) { const el = this[0]; const box = el.getBoundingClientRect(); const body = document.body; const clientTop = el.clientTop || body.clientTop || 0; const clientLeft = el.clientLeft || body.clientLeft || 0; const scrollTop = el === window ? window.scrollY : el.scrollTop; const scrollLeft = el === window ? window.scrollX : el.scrollLeft; return { top: (box.top + scrollTop) - clientTop, left: (box.left + scrollLeft) - clientLeft, } }
    return null
}
...
function each(callback) {
    ...
    for (let i = 0; i < this.length; i += 1) { if (callback.call(this[i], i, this[i]) === !1) { return this } }
    return this
}

function forEach(callback) {
    if (!callback) return this;
    for (let i = 0; i < this.length; i += 1) { if (callback.call(this[i], this[i], i) === !1) { return this } }
    return this
}
...
function is(selector) {
    ...
    if (typeof selector === 'string') {
        ...
        for (i = 0; i < compareWith.length; i += 1) { if (compareWith[i] === el) return !0 }
        return !1
...
function indexOf(el) {
    for (let i = 0; i < this.length; i += 1) { if (this[i] === el) return i }
    return -1
}
...
function children(selector) {
    const children = [];
    for (let i = 0; i < this.length; i += 1) { const childNodes = this[i].childNodes; for (let j = 0; j < childNodes.length; j += 1) { if (!selector) { if (childNodes[j].nodeType === 1) children.push(childNodes[j]) } else if (childNodes[j].nodeType === 1 && $(childNodes[j]).is(selector)) { children.push(childNodes[j]) } } }
...
function scrollTo(...args) {
    ...
    return this.each(function animate() {
        ...
        let startTime = null;
        if (animateTop && newTop === currentTop) animateTop = !1;
        if (animateLeft && newLeft === currentLeft) animateLeft = !1;
...
function scrollTop(...args) {
    ...
    const dom = this;
    if (typeof top === 'undefined') { if (dom.length > 0) return dom[0].scrollTop; return null }
...
function scrollLeft(...args) {
    ...
    const dom = this;
    if (typeof left === 'undefined') { if (dom.length > 0) return dom[0].scrollLeft; return null }
...
function animate(initialProps, initialParams) {
    ...
    const a = {
        ...
        animate(props, params) {
            if (a.animating) { a.que.push([props, params]); return a }
            const elements = [];
            a.elements.each((index, el) => {
                ...
                Object.keys(props).forEach((prop) => {
                    ...
                    elements[index][prop] = { initialFullValue, initialValue, unit, finalValue, finalFullValue, currentValue: initialValue, }
                })
            });
            let startTime = null;
            let time;
            ...
            function render() {
                ...
                let progress;
                let easeProgress;
                if (!began) { began = !0; if (params.begin) params.begin(els) }
                if (startTime === null) { startTime = time }
    ...
    let animateInstance;
    for (let i = 0; i < a.elements.length; i += 1) { if (a.elements[i].dom7AnimateInstance) { animateInstance = a.elements[i].dom7AnimateInstance } else a.elements[i].dom7AnimateInstance = a }
...
const Methods = { addClass, removeClass, hasClass, toggleClass, attr, removeAttr, data, transform, transition, on, off, trigger, transitionEnd, outerWidth, outerHeight, offset, css, each, html, text, is, index, eq, append, prepend, next, nextAll, prev, prevAll, parent, parents, closest, find, children, remove, add, styles, };
...
const Utils = {
    ...
    nextTick(callback, delay = 0) { return setTimeout(callback, delay) },
    now() { return Date.now() },
    ...
    isObject(o) { return typeof o === 'object' && o !== null && o.constructor && o.constructor === Object },
    extend(...args) {
...
const Support = (function Support() {
    ...
    return {
        touch: (window.Modernizr && window.Modernizr.touch === !0) || (function checkTouch() { return !!(('ontouchstart' in window) || (window.DocumentTouch && document instanceof window.DocumentTouch)) }()),
        pointerEvents: !!(window.navigator.pointerEnabled || window.PointerEvent),
        prefixedPointerEvents: !!window.navigator.msPointerEnabled,
        transition: (function checkTransition() { const style = testDiv.style; return ('transition' in style || 'webkitTransition' in style || 'MozTransition' in style) }()),
        transforms3d: (window.Modernizr && window.Modernizr.csstransforms3d === !0) || (function checkTransforms3d() { const style = testDiv.style; return ('webkitPerspective' in style || 'MozPerspective' in style || 'OPerspective' in style || 'MsPerspective' in style || 'perspective' in style) }()),
        flexbox: (function checkFlexbox() {
            const style = testDiv.style;
            const styles$$1 = ('alignItems webkitAlignItems webkitBoxAlign msFlexAlign mozBoxAlign webkitFlexDirection msFlexDirection mozBoxDirection mozBoxOrient webkitBoxDirection webkitBoxOrient').split(' ');
            for (let i = 0; i < styles$$1.length; i += 1) { if (styles$$1[i] in style) return !0 }
            return !1
        }()),
        ...
        passiveListener: (function checkPassiveListener() {
            ...
            try {
                const opts = Object.defineProperty({}, 'passive', {get() { supportsPassive = !0 }, });
...
class SwiperClass {
    ...
    off(events, handler) {
        ...
        events.split(' ').forEach((event) => { if (typeof handler === 'undefined') { self.eventsListeners[event] = [] } else if (self.eventsListeners[event] && self.eventsListeners[event].length) { self.eventsListeners[event].forEach((eventHandler, index$$1) => { if (eventHandler === handler) { self.eventsListeners[event].splice(index$$1, 1) } }) } });
        return self
    }
    emit(...args) {
        const self = this;
        ...
        eventsArray.forEach((event) => {
            if (self.eventsListeners && self.eventsListeners[event]) {
                ...
                handlers.forEach((eventHandler) => { eventHandler.apply(context, data$$1) })
            }
        });
        return self
    }
    useModulesParams(instanceParams) {
        ...
        Object.keys(instance.modules).forEach((moduleName) => { const module = instance.modules[moduleName]; if (module.params) { Utils.extend(instanceParams, module.params) } })
    }
    useModules(modulesParams = {}) {
        ...
        Object.keys(instance.modules).forEach((moduleName) => {
            ...
            if (module.instance) { Object.keys(module.instance).forEach((modulePropName) => { const moduleProp = module.instance[modulePropName]; if (typeof moduleProp === 'function') { instance[modulePropName] = moduleProp.bind(instance) } else { instance[modulePropName] = moduleProp } }) }
    ...
    static use(module, ...params) {
        const Class = this;
        if (Array.isArray(module)) { module.forEach(m => Class.installModule(m)); return Class }
...
function updateSlides() {
    ...
    for (let i = 0; i < slidesLength; i += 1) {
        ...
        } else { slideSize = (swiperSize - ((params.slidesPerView - 1) * spaceBetween)) / params.slidesPerView; if (params.roundLengths) slideSize = Math.floor(slideSize); if (slides[i]) { if (swiper.isHorizontal()) { slides[i].style.width = `${slideSize}px` } else { slides[i].style.height = `${slideSize}px` } } }
    ...
    if (params.slidesPerColumn > 1) {
        ...
        if (params.centeredSlides) {
            newSlidesGrid = [];
            for (let i = 0; i < snapGrid.length; i += 1) { let slidesGridItem = snapGrid[i]; if (params.roundLengths) slidesGridItem = Math.floor(slidesGridItem); if (snapGrid[i] < swiper.virtualSize + snapGrid[0]) newSlidesGrid.push(slidesGridItem) }
...
function transitionEnd$1(runCallbacks = !0, direction) {
    const swiper = this;
...
var transition$1 = { setTransition, transitionStart, transitionEnd: transitionEnd$1, };

function slideTo(index$$1 = 0, speed = this.params.speed, runCallbacks = !0, internal) {
    const swiper = this;
...
function slideToLoop(index$$1 = 0, speed = this.params.speed, runCallbacks = !0, internal) {
    const swiper = this;
...
function slideToClosest(speed = this.params.speed, runCallbacks = !0, internal) {
    const swiper = this;
    let index$$1 = swiper.activeIndex;
    const snapIndex = Math.floor(index$$1 / swiper.params.slidesPerGroup);
    if (snapIndex < swiper.snapGrid.length - 1) { const translate = swiper.rtlTranslate ? swiper.translate : -swiper.translate; const currentSnap = swiper.snapGrid[snapIndex]; const nextSnap = swiper.snapGrid[snapIndex + 1]; if ((translate - currentSnap) > (nextSnap - currentSnap) / 2) { index$$1 = swiper.params.slidesPerGroup } }
    ...
    if (params.loop) {
        ...
        if (params.centeredSlides) {
            if ((slideToIndex < swiper.loopedSlides - (slidesPerView / 2)) || (slideToIndex > (swiper.slides.length - swiper.loopedSlides) + (slidesPerView / 2))) {
                ...
                Utils.nextTick(() => { swiper.slideTo(slideToIndex) })
...
function addSlide(index$$1, slides) {
    ...
    if (index$$1 <= 0) { swiper.prependSlide(slides); return }
...
const Device = (function Device() {
    ...
    const device = { ios: !1, android: !1, androidChrome: !1, desktop: !1, windows: !1, iphone: !1, ipod: !1, ipad: !1, cordova: window.cordova || window.phonegap, phonegap: window.cordova || window.phonegap, };
...
function onTouchStart(event) {
    ...
    Utils.extend(data$$1, { isTouched: !0, isMoved: !1, allowTouchCallbacks: !0, isScrolling: undefined, startMoving: undefined, });
    ...
    if (params.threshold > 0) data$$1.allowThresholdMove = !1;
    if (e.type !== 'touchstart') {
        let preventDefault = !0;
        if ($(e.target).is(data$$1.formElements)) preventDefault = !1;
        if (document.activeElement && $(document.activeElement).is(data$$1.formElements) && document.activeElement !== e.target) { document.activeElement.blur() }
...
function onTouchMove(event) {
    ...
    if (typeof data$$1.isScrolling === 'undefined') {
        let touchAngle;
        if ((swiper.isHorizontal() && touches.currentY === touches.startY) || (swiper.isVertical() && touches.currentX === touches.startX)) { data$$1.isScrolling = !1 } else {
    ...
    if (params.threshold > 0) {
        ...
        } else { data$$1.currentTranslate = data$$1.startTranslate; return }
    }
    ...
    if (params.freeMode) {
        if (data$$1.velocities.length === 0) { data$$1.velocities.push({ position: touches[swiper.isHorizontal() ? 'startX' : 'startY'], time: data$$1.touchStartTime, }) }
...
function onTouchEnd(event) {
    ...
    data$$1.allowTouchCallbacks = !1;
    if (!data$$1.isTouched) {
        if (data$$1.isMoved && params.grabCursor) { swiper.setGrabCursor(!1) }
        data$$1.isMoved = !1;
        data$$1.startMoving = !1;
        return
    }
    ...
    Utils.nextTick(() => { if (!swiper.destroyed) swiper.allowClick = !0 });
    ...
    data$$1.startMoving = !1;
    let currentPos;
    if (params.followFinger) { currentPos = rtl ? swiper.translate : -swiper.translate } else { currentPos = -data$$1.currentTranslate }
    if (params.freeMode) {
...
function onResize() {
    ...
    if (params.freeMode) {
        ...
        swiper.updateSlidesClasses();
        if (params.autoHeight) { swiper.updateAutoHeight() }
    } else { swiper.updateSlidesClasses(); if ((params.slidesPerView === 'auto' || params.slidesPerView > 1) && swiper.isEnd && !swiper.params.centeredSlides) { swiper.slideTo(swiper.slides.length - 1, 0, !1, !0) } else { swiper.slideTo(swiper.activeIndex, 0, !1, !0) } }
    swiper.allowSlidePrev = allowSlidePrev;
    swiper.allowSlideNext = allowSlideNext;
...
function attachEvents() {
    ...
    const capture = !!params.nested; {
        if (!Support.touch && (Support.pointerEvents || Support.prefixedPointerEvents)) {
            ...
            document.addEventListener(touchEvents.end, swiper.onTouchEnd, !1)
        } else {
            if (Support.touch) {
                const passiveListener = touchEvents.start === 'touchstart' && Support.passiveListener && params.passiveListeners ? { passive: !0, capture: !1 } : !1;
                target.addEventListener(touchEvents.start, swiper.onTouchStart, passiveListener);
                target.addEventListener(touchEvents.move, swiper.onTouchMove, Support.passiveListener ? { passive: !1, capture } : capture);
...
function detachEvents() {
    ...
    const capture = !!params.nested; {
        if (!Support.touch && (Support.pointerEvents || Support.prefixedPointerEvents)) {
            ...
            document.removeEventListener(touchEvents.end, swiper.onTouchEnd, !1)
        } else {
            if (Support.touch) {
                const passiveListener = touchEvents.start === 'onTouchStart' && Support.passiveListener && params.passiveListeners ? { passive: !0, capture: !1 } : !1;
...
function getBreakpoint(breakpoints) {
    ...
    for (let i = 0; i < points.length; i += 1) { const point = points[i]; if (swiper.params.breakpointsInverse) { if (point <= window.innerWidth) { breakpoint = point } } else if (point >= window.innerWidth && !breakpoint) { breakpoint = point } }
...
const prototypes = { update, translate, transition: transition$1, slide, loop, grabCursor, manipulation, events, breakpoints, checkOverflow: checkOverflow$1, classes, images, };
...
class Swiper extends SwiperClass {
    ...
    slidesPerViewDynamic() {
        ...
        } else { for (let i = activeIndex + 1; i < slides.length; i += 1) { if (slidesGrid[i] - slidesGrid[activeIndex] < swiperSize) { spv += 1 } } }
        return spv
    }
    update() {
        const swiper = this;
...
const Observer = {
    ...
    attach(target, options = {}) {
        ...
        observer.observe(target, { attributes: typeof options.attributes === 'undefined' ? !0 : options.attributes, childList: typeof options.childList === 'undefined' ? !0 : options.childList, characterData: typeof options.characterData === 'undefined' ? !0 : options.characterData, });
...
const Virtual = {
    update(force) {
        ...
        Utils.extend(swiper.virtual, { from, to, offset: offset$$1, slidesGrid: swiper.slidesGrid, });
        ...
        appendIndexes.forEach((index$$1) => { swiper.$wrapperEl.append(renderSlide(slides[index$$1], index$$1)) });
        prependIndexes.sort((a, b) => a < b).forEach((index$$1) => { swiper.$wrapperEl.prepend(renderSlide(slides[index$$1], index$$1)) });
    ...
    prependSlide(slide) {
        ...
        if (swiper.params.virtual.cache) {
            ...
            const newCache = {};
            Object.keys(cache).forEach((cachedIndex) => { newCache[cachedIndex + 1] = cache[cachedIndex] });
...
const Keyboard = {
    handle(event) {
        ...
        if (!swiper.allowSlideNext && ((swiper.isHorizontal() && kc === 39) || (swiper.isVertical() && kc === 40))) { return !1 }
        ...
        if (swiper.params.keyboard.onlyInViewport && (kc === 37 || kc === 39 || kc === 38 || kc === 40)) {
            ...
            for (let i = 0; i < swiperCoord.length; i += 1) { const point = swiperCoord[i]; if (point[0] >= 0 && point[0] <= windowWidth && point[1] >= 0 && point[1] <= windowHeight) { inView = !0 } }
...
var Keyboard$1 = {
    ...
    on: { init() { const swiper = this; if (swiper.params.keyboard.enabled) { swiper.keyboard.enable() } }, destroy() { const swiper = this; if (swiper.keyboard.enabled) { swiper.keyboard.disable() } }, },
};
...
const Mousewheel = {
    ...
    event: (function getEvent() { if (window.navigator.userAgent.indexOf('firefox') > -1) return 'DOMMouseScroll'; return isEventSupported() ? 'wheel' : 'mousewheel' }()),
    normalize(e) {
        const PIXEL_STEP = 10;
        ...
        if (!swiper.params.freeMode) {
            if (Utils.now() - swiper.mousewheel.lastScrollTime > 60) {
                if (delta < 0) {
                    ...
                    } else if (params.releaseOnEdges) return !0
...
var Mousewheel$1 = {
    ...
    create() {
        const swiper = this;
        Utils.extend(swiper, { mousewheel: { enabled: !1, enable: Mousewheel.enable.bind(swiper), disable: Mousewheel.disable.bind(swiper), handle: Mousewheel.handle.bind(swiper), handleMouseEnter: Mousewheel.handleMouseEnter.bind(swiper), handleMouseLeave: Mousewheel.handleMouseLeave.bind(swiper), lastScrollTime: Utils.now(), }, })
    },
    on: { init() { const swiper = this; if (swiper.params.mousewheel.enabled) swiper.mousewheel.enable() }, destroy() { const swiper = this; if (swiper.mousewheel.enabled) swiper.mousewheel.disable() }, },
};
const Navigation = {
    update() {
        const swiper = this;
    ...
    init() {
        ...
        Utils.extend(swiper.navigation, { $nextEl, nextEl: $nextEl && $nextEl[0], $prevEl, prevEl: $prevEl && $prevEl[0], })
    },
    destroy() {
        const swiper = this;
        const { $nextEl, $prevEl } = swiper.navigation;
...
var Navigation$1 = {
    ...
    on: {
        ...
        click(e) { const swiper = this; const { $nextEl, $prevEl } = swiper.navigation; if (swiper.params.navigation.hideOnClick && !$(e.target).is($prevEl) && !$(e.target).is($nextEl)) { if ($nextEl) $nextEl.toggleClass(swiper.params.navigation.hiddenClass); if ($prevEl) $prevEl.toggleClass(swiper.params.navigation.hiddenClass) } },
    },
};
const Pagination = {
    update() {
    ...
    init() {
        ...
        Utils.extend(swiper.pagination, { $el, el: $el[0], })
    },
    destroy() {
        const swiper = this;
        const params = swiper.params.pagination;
...
var Pagination$1 = {
    ...
    on: {
        ...
        activeIndexChange() { const swiper = this; if (swiper.params.loop) { swiper.pagination.update() } else if (typeof swiper.snapIndex === 'undefined') { swiper.pagination.update() } },
        snapIndexChange() { const swiper = this; if (!swiper.params.loop) { swiper.pagination.update() } },
        slidesLengthChange() {
            const swiper = this;
            if (swiper.params.loop) {
                swiper.pagination.render();
        ...
        click(e) { const swiper = this; if (swiper.params.pagination.el && swiper.params.pagination.hideOnClick && swiper.pagination.$el.length > 0 && !$(e.target).hasClass(swiper.params.pagination.bulletClass)) { swiper.pagination.$el.toggleClass(swiper.params.pagination.hiddenClass) } },
    },
};
const Scrollbar = {
    setTranslate() {
    ...
    enableDraggable() {
        ...
        const target = $el[0];
        const activeListener = Support.passiveListener && params.passiveListeners ? { passive: !1, capture: !1 } : !1;
        const passiveListener = Support.passiveListener && params.passiveListeners ? { passive: !0, capture: !1 } : !1;
        if (!Support.touch && (Support.pointerEvents || Support.prefixedPointerEvents)) {
...
const Parallax = {
    ...
    setTranslate() {
        ...
        $el.children('[data-swiper-parallax], [data-swiper-parallax-x], [data-swiper-parallax-y]').each((index$$1, el) => { swiper.parallax.setTransform(el, progress) });
        slides.each((slideIndex, slideEl) => {
            let slideProgress = slideEl.progress;
            if (swiper.params.slidesPerGroup > 1 && swiper.params.slidesPerView !== 'auto') { slideProgress += Math.ceil(slideIndex / 2) - (progress * (snapGrid.length - 1)) }
            slideProgress = Math.min(Math.max(slideProgress, -1), 1);
            $(slideEl).find('[data-swiper-parallax], [data-swiper-parallax-x], [data-swiper-parallax-y]').each((index$$1, el) => { swiper.parallax.setTransform(el, slideProgress) })
        })
    },
...
const Zoom = {
    getDistanceBetweenTouches(e) { if (e.targetTouches.length < 2) return 1; const x1 = e.targetTouches[0].pageX; const y1 = e.targetTouches[0].pageY; const x2 = e.targetTouches[1].pageX; const y2 = e.targetTouches[1].pageY; const distance = Math.sqrt(((x2 - x1) ** 2) + ((y2 - y1) ** 2)); return distance },
    onGestureStart(e) {
        const swiper = this;
    ...
    toggle(e) { const swiper = this; const zoom = swiper.zoom; if (zoom.scale && zoom.scale !== 1) { zoom.out() } else { zoom.in(e) } },
    in (e) {
        const swiper = this;
        const zoom = swiper.zoom;
        const params = swiper.params.zoom;
    ...
    enable() {
        ...
        zoom.enabled = !0;
        const passiveListener = swiper.touchEvents.start === 'touchstart' && Support.passiveListener && swiper.params.passiveListeners ? { passive: !0, capture: !1 } : !1;
        if (Support.gestures) {
...
var Zoom$1 = {
    ...
    create() {
        const swiper = this;
        const zoom = { enabled: !1, scale: 1, currentScale: 1, isScaling: !1, gesture: { $slideEl: undefined, slideWidth: undefined, slideHeight: undefined, $imageEl: undefined, $imageWrapEl: undefined, maxRatio: 3, }, image: { isTouched: undefined, isMoved: undefined, currentX: undefined, currentY: undefined, minX: undefined, minY: undefined, maxX: undefined, maxY: undefined, width: undefined, height: undefined, startX: undefined, startY: undefined, touchesStart: {}, touchesCurrent: {}, }, velocity: { x: undefined, y: undefined, prevPositionX: undefined, prevPositionY: undefined, prevTime: undefined, }, };
        ('onGestureStart onGestureChange onGestureEnd onTouchStart onTouchMove onTouchEnd onTransitionEnd toggle enable disable in out').split(' ').forEach((methodName) => { zoom[methodName] = Zoom[methodName].bind(swiper) });
        Utils.extend(swiper, { zoom, })
    },
    on: {
        init() { const swiper = this; if (swiper.params.zoom.enabled) { swiper.zoom.enable() } },
        destroy() {
            const swiper = this;
            swiper.zoom.disable()
        },
        ...
        doubleTap(e) { const swiper = this; if (swiper.params.zoom.enabled && swiper.zoom.enabled && swiper.params.zoom.toggle) { swiper.zoom.toggle(e) } },
        transitionEnd() { const swiper = this; if (swiper.zoom.enabled && swiper.params.zoom.enabled) { swiper.zoom.onTransitionEnd() } },
    },
};
const Lazy = {
    loadInSlide(index$$1, loadInDuplicate = !0) {
        const swiper = this;
    ...
    load() {
        ...
        } else if (slidesPerView > 1) { for (let i = activeIndex; i < activeIndex + slidesPerView; i += 1) { if (slideExist(i)) swiper.lazy.loadInSlide(i) } } else { swiper.lazy.loadInSlide(activeIndex) }
        if (params.loadPrevNext) {
            ...
            } else { const nextSlide = $wrapperEl.children(`.${swiperParams.slideNextClass}`); if (nextSlide.length > 0) swiper.lazy.loadInSlide(slideIndex(nextSlide)); const prevSlide = $wrapperEl.children(`.${swiperParams.slidePrevClass}`); if (prevSlide.length > 0) swiper.lazy.loadInSlide(slideIndex(prevSlide)) }
        }
    },
};
var Lazy$1 = {
    ...
    on: { beforeInit() { const swiper = this; if (swiper.params.lazy.enabled && swiper.params.preloadImages) { swiper.params.preloadImages = !1 } }, init() { const swiper = this; if (swiper.params.lazy.enabled && !swiper.params.loop && swiper.params.initialSlide === 0) { swiper.lazy.load() } }, scroll() { const swiper = this; if (swiper.params.freeMode && !swiper.params.freeModeSticky) { swiper.lazy.load() } }, resize() { const swiper = this; if (swiper.params.lazy.enabled) { swiper.lazy.load() } }, scrollbarDragMove() { const swiper = this; if (swiper.params.lazy.enabled) { swiper.lazy.load() } }, transitionStart() { const swiper = this; if (swiper.params.lazy.enabled) { if (swiper.params.lazy.loadOnTransitionStart || (!swiper.params.lazy.loadOnTransitionStart && !swiper.lazy.initialImageLoaded)) { swiper.lazy.load() } } }, transitionEnd() { const swiper = this; if (swiper.params.lazy.enabled && !swiper.params.lazy.loadOnTransitionStart) { swiper.lazy.load() } }, },
};
const Controller = {
    ...
    getInterpolateFunction(c) { const swiper = this; if (!swiper.controller.spline) { swiper.controller.spline = swiper.params.loop ? new Controller.LinearSpline(swiper.slidesGrid, c.slidesGrid) : new Controller.LinearSpline(swiper.snapGrid, c.snapGrid) } },
...
const a11y = {
    makeElFocusable($el) { $el.attr('tabIndex', '0'); return $el },
    addElRole($el, role) { $el.attr('role', role); return $el },
    addElLabel($el, label) { $el.attr('aria-label', label); return $el },
    disableEl($el) { $el.attr('aria-disabled', !0); return $el },
    enableEl($el) { $el.attr('aria-disabled', !1); return $el },
    onEnterKey(e) {
        const swiper = this;
...
var A11y = {
    ...
    create() {
        ...
        Object.keys(a11y).forEach((methodName) => { swiper.a11y[methodName] = a11y[methodName].bind(swiper) })
    },
    on: {
        init() {
            const swiper = this;
...
const History = {
    ...
    destroy() { const swiper = this; if (!swiper.params.history.replaceState) { window.removeEventListener('popstate', swiper.history.setHistoryPopState) } },
    setHistoryPopState() {
        const swiper = this;
        swiper.history.paths = History.getPathValues();
        swiper.history.scrollToSlide(swiper.params.speed, swiper.history.paths.value, !1)
    },
    getPathValues() { const pathArray = window.location.pathname.slice(1).split('/').filter(part => part !== ''); const total = pathArray.length; const key = pathArray[total - 2]; const value = pathArray[total - 1]; return { key, value } },
    ...
    slugify(text$$1) { return text$$1.toString().toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '').replace(/--+/g, '-').replace(/^-+/, '').replace(/-+$/, '') },
...
var History$1 = {
    ...
    on: { init() { const swiper = this; if (swiper.params.history.enabled) { swiper.history.init() } }, destroy() { const swiper = this; if (swiper.params.history.enabled) { swiper.history.destroy() } }, transitionEnd() { const swiper = this; if (swiper.history.initialized) { swiper.history.setHistory(swiper.params.history.key, swiper.activeIndex) } }, },
};
const HashNavigation = {
    ...
    setHash() {
        ...
        if (swiper.params.hashNavigation.replaceState && window.history && window.history.replaceState) { window.history.replaceState(null, null, (`#${swiper.slides.eq(swiper.activeIndex).attr('data-hash')}` || '')) } else {
        ...
        if (hash) {
            ...
            for (let i = 0, length = swiper.slides.length; i < length; i += 1) {
                ...
                if (slideHash === hash && !slide.hasClass(swiper.params.slideDuplicateClass)) {
                    const index$$1 = slide.index();
                    swiper.slideTo(index$$1, speed, swiper.params.runCallbacksOnInit, !0)
                }
            }
        }
        if (swiper.params.hashNavigation.watchState) { $(window).on('hashchange', swiper.hashNavigation.onHashCange) }
    },
    destroy() { const swiper = this; if (swiper.params.hashNavigation.watchState) { $(window).off('hashchange', swiper.hashNavigation.onHashCange) } },
};
var HashNavigation$1 = {
    ...
    create() {
        const swiper = this;
        Utils.extend(swiper, { hashNavigation: { initialized: !1, init: HashNavigation.init.bind(swiper), destroy: HashNavigation.destroy.bind(swiper), setHash: HashNavigation.setHash.bind(swiper), onHashCange: HashNavigation.onHashCange.bind(swiper), }, })
    },
    on: { init() { const swiper = this; if (swiper.params.hashNavigation.enabled) { swiper.hashNavigation.init() } }, destroy() { const swiper = this; if (swiper.params.hashNavigation.enabled) { swiper.hashNavigation.destroy() } }, transitionEnd() { const swiper = this; if (swiper.hashNavigation.initialized) { swiper.hashNavigation.setHash() } }, },
};
const Autoplay = {
    run() {
        const swiper = this;
...
const Thumbs = {
    init() {
        ...
        if (thumbsParams.swiper instanceof SwiperClass) {
            ...
            swiper.thumbs.swiper = new SwiperClass(Utils.extend({}, thumbsParams.swiper, { watchSlidesVisibility: !0, watchSlidesProgress: !0, slideToClickedSlide: !1, }));
            swiper.thumbs.swiperCreated = !0
        }
    ...
    update(initial) {
        ...
        if (swiper.realIndex !== thumbsSwiper.realIndex) {
            ...
            if (thumbsSwiper.visibleSlidesIndexes.indexOf(newThumbsIndex) < 0) {
                if (thumbsSwiper.params.centeredSlides) { if (newThumbsIndex > currentThumbsIndex) { newThumbsIndex = newThumbsIndex - Math.floor(slidesPerView / 2) + 1 } else { newThumbsIndex = newThumbsIndex + Math.floor(slidesPerView / 2) - 1 } } else if (newThumbsIndex > currentThumbsIndex) { newThumbsIndex = newThumbsIndex - slidesPerView + 1 }
...
var Thumbs$1 = {
    ...
    on: {
        ...
        beforeDestroy() { const swiper = this; const thumbsSwiper = swiper.thumbs.swiper; if (!thumbsSwiper) return; if (swiper.thumbs.swiperCreated && thumbsSwiper) { thumbsSwiper.destroy() } },
    },
};
