const directives = new WeakMap,
    ...
    rewritesStyleAttribute = (() => { const e = document.createElement("div"); return e.setAttribute("style", "{{bad value}}"), "{{bad value}}" !== e.getAttribute("style") })();
class Template {
    constructor(e, t) {
        ...
        const o = [],
            n = t => {
                ...
                for (; a.nextNode();) {
                    i++, l = c;
                    const t = c = a.currentNode;
                    if (1 === t.nodeType) {
                        if (t.hasAttributes()) {
                            const o = t.attributes;
                            let n = 0;
                            for (let e = 0; e < o.length; e++) o[e].value.indexOf(marker) >= 0 && n++;
                            for (; n-- > 0;) {
                                const o = e.strings[r],
                                    n = lastAttributeNameRegex.exec(o)[2],
                                    s = rewritesStyleAttribute && "style" === n ? "style$" : /^[a-zA-Z-]*$/.test(n) ? n : n.toLowerCase(),
                                    a = t.getAttribute(s).split(markerRegex);
                                this.parts.push({ type: "attribute", index: i, name: n, strings: a }), t.removeAttribute(s), r += a.length - 1
                            }
                        }
                        "TEMPLATE" === t.tagName && n(t)
                    } else if (3 === t.nodeType) {
                        const e = t.nodeValue;
                        if (e.indexOf(marker) < 0) continue;
                        const n = t.parentNode,
                            s = e.split(markerRegex),
                            a = s.length - 1;
                        r += a;
                        for (let e = 0; e < a; e++) n.insertBefore("" === s[e] ? createMarker() : document.createTextNode(s[e]), t), this.parts.push({ type: "node", index: i++ });
                        n.insertBefore("" === s[a] ? createMarker() : document.createTextNode(s[a]), t), o.push(t)
                    } else if (8 === t.nodeType)
                        if (t.nodeValue === marker) {
                            const e = t.parentNode,
                                n = t.previousSibling;
                            null === n || n !== l || n.nodeType !== Node.TEXT_NODE ? e.insertBefore(createMarker(), t) : i--, this.parts.push({ type: "node", index: i++ }), o.push(t), null === t.nextSibling ? e.insertBefore(createMarker(), t) : i--, c = l, r++
                        } else { let e = -1; for (; - 1 !== (e = t.nodeValue.indexOf(marker, e + 1));) this.parts.push({ type: "node", index: -1 }) }
                }
            };
        n(t);
...
const isTemplatePartActive = e => -1 !== e.index,
    createMarker = () => document.createComment(""),
    lastAttributeNameRegex = /([ \x09\x0a\x0c\x0d])([^\0-\x1F\x7F-\x9F \x09\x0a\x0c\x0d"'>=\/]+)([ \x09\x0a\x0c\x0d]*=[ \x09\x0a\x0c\x0d]*(?:[^ \x09\x0a\x0c\x0d"'`<>=]*|"[^"]*|'[^']*))$/;
class TemplateInstance {
    constructor(e, t, i) { this._parts = [], this.template = e, this.processor = t, this.options = i }
    update(e) { let t = 0; for (const i of this._parts) void 0 !== i && i.setValue(e[t]), t++; for (const e of this._parts) void 0 !== e && e.commit() }
    _clone() {
        ...
        const o = e => {
            ...
            let s = n.nextNode();
            for (; i < t.length && null !== s;) {
                const e = t[i];
                if (isTemplatePartActive(e))
                    if (r === e.index) {
                        if ("node" === e.type) {
                            const e = this.processor.handleTextExpression(this.options);
                            e.insertAfterNode(s), this._parts.push(e)
                        } else this._parts.push(...this.processor.handleAttributeExpressions(s, e.name, e.strings, this.options));
                        i++
                    } else r++, "TEMPLATE" === s.nodeName && o(s.content), s = n.nextNode();
                else this._parts.push(void 0), i++
            }
        };
        return o(e), isCEPolyfill && (document.adoptNode(e), customElements.upgrade(e)), e
    }
}
class TemplateResult {
    constructor(e, t, i, r) { this.strings = e, this.values = t, this.type = i, this.processor = r }
    getHTML() {
        ...
        for (let r = 0; r < e; r++) {
            ...
            !(i = (o > -1 || i) && -1 === e.indexOf("<", o + 1)) && rewritesStyleAttribute && (t = t.replace(lastAttributeNameRegex, (e, t, i, r) => "style" === i ? `${t}style$${r}` : e)), t += i ? nodeMarker : marker
        }
...
class AttributePart {
    ...
    setValue(e) { e === noChange || isPrimitive(e) && e === this.value || (this.value = e, isDirective(e) || (this.committer.dirty = !0)) }
    commit() {
        for (; isDirective(this.value);) {
            const e = this.value;
            this.value = noChange, e(this)
        }
        this.value !== noChange && this.committer.commit()
    }
}
class NodePart {
    constructor(e) { this.value = void 0, this._pendingValue = void 0, this.options = e }
    ...
    insertAfterPart(e) { e._insert(this.startNode = createMarker()), this.endNode = e.endNode, e.endNode = this.startNode }
    setValue(e) { this._pendingValue = e }
    commit() {
        for (; isDirective(this._pendingValue);) {
            const e = this._pendingValue;
            this._pendingValue = noChange, e(this)
        }
        const e = this._pendingValue;
        e !== noChange && (isPrimitive(e) ? e !== this.value && this._commitText(e) : e instanceof TemplateResult ? this._commitTemplateResult(e) : e instanceof Node ? this._commitNode(e) : Array.isArray(e) || e[Symbol.iterator] ? this._commitIterable(e) : void 0 !== e.then ? this._commitPromise(e) : this._commitText(e))
    }
    _insert(e) { this.endNode.parentNode.insertBefore(e, this.endNode) }
    _commitNode(e) { this.value !== e && (this.clear(), this._insert(e), this.value = e) }
    _commitText(e) {
        const t = this.startNode.nextSibling;
        e = null == e ? "" : e, t === this.endNode.previousSibling && t.nodeType === Node.TEXT_NODE ? t.textContent = e : this._commitNode(document.createTextNode("string" == typeof e ? e : String(e))), this.value = e
    }
    _commitTemplateResult(e) {
        const t = this.options.templateFactory(e);
        if (this.value && this.value.template === t) this.value.update(e.values);
        else {
            const i = new TemplateInstance(t, e.processor, this.options),
                r = i._clone();
            i.update(e.values), this._commitNode(r), this.value = i
        }
    }
    _commitIterable(e) {
        ...
        const t = this.value;
        let i, r = 0;
        for (const o of e) void 0 === (i = t[r]) && (i = new NodePart(this.options), t.push(i), 0 === r ? i.appendIntoPart(this) : i.insertAfterPart(t[r - 1])), i.setValue(o), i.commit(), r++;
        r < t.length && (t.length = r, this.clear(i && i.endNode))
    }
    _commitPromise(e) { this.value = e, e.then(t => { this.value === e && (this.setValue(t), this.commit()) }) }
    clear(e = this.startNode) { removeNodes(this.startNode.parentNode, e.nextSibling, this.endNode) }
}
class BooleanAttributePart {
    ...
    commit() {
        ...
        const e = !!this._pendingValue;
        this.value !== e && (e ? this.element.setAttribute(this.name, "") : this.element.removeAttribute(this.name)), this.value = e, this._pendingValue = noChange
    }
}
class PropertyCommitter extends AttributeCommitter {
    constructor(e, t, i) { super(e, t, i), this.single = 2 === i.length && "" === i[0] && "" === i[1] }
...
try {
    const e = {get capture() { return eventOptionsSupported = !0, !1 } };
    window.addEventListener("test", e, e), window.removeEventListener("test", e, e)
} catch (e) {}
class EventPart {
    constructor(e, t, i) { this.value = void 0, this._pendingValue = void 0, this.element = e, this.eventName = t, this.eventContext = i }
    setValue(e) { this._pendingValue = e }
    commit() {
        for (; isDirective(this._pendingValue);) {
            const e = this._pendingValue;
        ...
        const e = this._pendingValue,
            ...
            r = null != e && (null == t || i);
        i && this.element.removeEventListener(this.eventName, this, this._options), this._options = getOptions(e), r && this.element.addEventListener(this.eventName, this, this._options), this.value = e, this._pendingValue = noChange
    }
    handleEvent(e) {
        ("function" == typeof this.value ? this.value : "function" == typeof this.value.handleEvent ? this.value.handleEvent : () => null).call(this.eventContext || this.element, e)
    }
}
const getOptions = e => e && (eventOptionsSupported ? { capture: e.capture, passive: e.passive, once: e.once } : e.capture);
class DefaultTemplateProcessor {
    handleAttributeExpressions(e, t, i, r) { const o = t[0]; if ("." === o) { return new PropertyCommitter(e, t.slice(1), i).parts } return "@" === o ? [new EventPart(e, t.slice(1), r.eventContext)] : "?" === o ? [new BooleanAttributePart(e, t.slice(1), i)] : new AttributeCommitter(e, t, i).parts }
    handleTextExpression(e) { return new NodePart(e) }
}
const defaultTemplateProcessor = new DefaultTemplateProcessor;

function templateFactory(e) {
    let t = templateCaches.get(e.type);
    void 0 === t && (t = new Map, templateCaches.set(e.type, t));
    let i = t.get(e.strings);
    return void 0 === i && (i = new Template(e, e.getTemplateElement()), t.set(e.strings, i)), i
}
const templateCaches = new Map,
    parts = new WeakMap,
    render = (e, t, i) => {
        let r = parts.get(t);
        void 0 === r && (removeNodes(t, t.firstChild), parts.set(t, r = new NodePart(Object.assign({ templateFactory: templateFactory }, i))), r.appendInto(t)), r.setValue(e), r.commit()
    },
    html = (e, ...t) => new TemplateResult(e, t, "html", defaultTemplateProcessor),
    walkerNodeFilter = NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_COMMENT | NodeFilter.SHOW_TEXT;

function removeNodesFromTemplate(e, t) {
    const { element: { content: i }, parts: r } = e, o = document.createTreeWalker(i, walkerNodeFilter, null, !1);
    ...
    const c = [];
    let h = null;
    for (; o.nextNode();) { a++; const e = o.currentNode; for (e.previousSibling === h && (h = null), t.has(e) && (c.push(e), null === h && (h = e)), null !== h && l++; void 0 !== s && s.index === a;) s.index = null !== h ? -1 : s.index - l, s = r[n = nextActiveIndexInTemplateParts(r, n)] }
    c.forEach(e => e.parentNode.removeChild(e))
}
const countNodes = e => { let t = e.nodeType === Node.DOCUMENT_FRAGMENT_NODE ? 0 : 1; const i = document.createTreeWalker(e, walkerNodeFilter, null, !1); for (; i.nextNode();) t++; return t },
    nextActiveIndexInTemplateParts = (e, t = -1) => { for (let i = t + 1; i < e.length; i++) { const t = e[i]; if (isTemplatePartActive(t)) return i } return -1 };

function insertNodeIntoTemplate(e, t, i = null) {
    const { element: { content: r }, parts: o } = e;
    if (null == i) return void r.appendChild(t);
    const n = document.createTreeWalker(r, walkerNodeFilter, null, !1);
    let s = nextActiveIndexInTemplateParts(o),
        a = 0,
        l = -1;
    for (; n.nextNode();) {
        for (l++, n.currentNode === i && (a = countNodes(t), i.parentNode.insertBefore(t, i)); - 1 !== s && o[s].index === l;) {
            if (a > 0) { for (; - 1 !== s;) o[s].index += a, s = nextActiveIndexInTemplateParts(o, s); return }
            s = nextActiveIndexInTemplateParts(o, s)
        }
    }
}
...
const shadyTemplateFactory = e => t => {
        const i = getTemplateCacheKey(t.type, e);
        let r = templateCaches.get(i);
        void 0 === r && (r = new Map, templateCaches.set(i, r));
        let o = r.get(t.strings);
        if (void 0 === o) {
            const i = t.getTemplateElement();
            compatibleShadyCSSVersion && window.ShadyCSS.prepareTemplateDom(i, e), o = new Template(t, i), r.set(t.strings, o)
        }
        return o
    },
    ...
    removeStylesFromLitTemplates = e => {
        TEMPLATE_TYPES.forEach(t => {
            const i = templateCaches.get(getTemplateCacheKey(t, e));
            void 0 !== i && i.forEach(e => {
                const { element: { content: t } } = e, i = new Set;
                Array.from(t.querySelectorAll("style")).forEach(e => { i.add(e) }), removeNodesFromTemplate(e, i)
            })
        })
    },
    shadyRenderSet = new Set,
    prepareTemplateStyles = (e, t, i) => {
        shadyRenderSet.add(i);
        ...
        if (removeStylesFromLitTemplates(i), insertNodeIntoTemplate(t, o, t.element.content.firstChild), window.ShadyCSS.prepareTemplateStyles(t.element, i), window.ShadyCSS.nativeShadow) {
            const i = t.element.content.querySelector("style");
            e.insertBefore(i.cloneNode(!0), e.firstChild)
        } else {
            t.element.content.insertBefore(o, t.element.content.firstChild);
            const e = new Set;
            e.add(o), removeNodesFromTemplate(t, e)
        }
    },
    render$1 = (e, t, i) => {
        const r = i.scopeName,
            o = parts.has(t);
        if (render(e, t, Object.assign({ templateFactory: shadyTemplateFactory(r) }, i)), t instanceof ShadowRoot && compatibleShadyCSSVersion && e instanceof TemplateResult) {
            if (!shadyRenderSet.has(r)) {
                const e = parts.get(t).value;
                prepareTemplateStyles(t, e.template, r)
            }
            o || window.ShadyCSS.styleElement(t.host)
        }
    },
    fromBooleanAttribute = e => null !== e,
    toBooleanAttribute = e => e ? "" : null,
    notEqual = (e, t) => t !== e && (t == t || e == e),
    defaultPropertyDeclaration = { attribute: !0, type: String, reflect: !1, hasChanged: notEqual },
    microtaskPromise = new Promise(e => e(!0)),
    STATE_HAS_UPDATED = 1,
    STATE_UPDATE_REQUESTED = 4,
    STATE_IS_REFLECTING = 8;
class UpdatingElement extends HTMLElement {
    constructor() { super(), this._updateState = 0, this._instanceProperties = void 0, this._updatePromise = microtaskPromise, this._changedProperties = new Map, this._reflectingProperties = void 0, this.initialize() }
    static get observedAttributes() {
        this._finalize();
        const e = [];
        for (const [t, i] of this._classProperties) {
            const r = this._attributeNameForProperty(t, i);
            void 0 !== r && (this._attributeToPropertyMap.set(r, t), e.push(r))
        }
        return e
    }
    static createProperty(e, t = defaultPropertyDeclaration) {
        ...
        Object.defineProperty(this.prototype, e, {get() { return this[i] },
            set(r) {
                const o = this[e];
                this[i] = r, this._requestPropertyUpdate(e, o, t)
            },
            configurable: !0,
            enumerable: !0
        })
    ...
    static _attributeNameForProperty(e, t) { const i = void 0 !== t && t.attribute; return !1 === i ? void 0 : "string" == typeof i ? i : "string" == typeof e ? e.toLowerCase() : void 0 }
    static _valueHasChanged(e, t, i = notEqual) { return i(e, t) }
    static _propertyValueFromAttribute(e, t) { const i = t && t.type; if (void 0 === i) return e; const r = i === Boolean ? fromBooleanAttribute : "function" == typeof i ? i : i.fromAttribute; return r ? r(e) : e }
    static _propertyValueToAttribute(e, t) { if (void 0 === t || void 0 === t.reflect) return; return (t.type === Boolean ? toBooleanAttribute : t.type && t.type.toAttribute || String)(e) }
    ...
    connectedCallback() { this._updateState & STATE_HAS_UPDATED ? void 0 !== window.ShadyCSS && window.ShadyCSS.styleElement(this) : this.requestUpdate() }
    disconnectedCallback() {}
    attributeChangedCallback(e, t, i) { t !== i && this._attributeToProperty(e, i) }
    _propertyToAttribute(e, t, i = defaultPropertyDeclaration) {
        const r = this.constructor,
            o = r._propertyValueToAttribute(t, i);
        if (void 0 !== o) {
            const t = r._attributeNameForProperty(e, i);
            void 0 !== t && (this._updateState = this._updateState | STATE_IS_REFLECTING, null === o ? this.removeAttribute(t) : this.setAttribute(t, o), this._updateState = this._updateState & ~STATE_IS_REFLECTING)
        }
    }
    _attributeToProperty(e, t) {
        if (!(this._updateState & STATE_IS_REFLECTING)) {
            ...
            if (void 0 !== r) {
                const e = i._classProperties.get(r);
                this[r] = i._propertyValueFromAttribute(t, e)
            }
        }
    }
    requestUpdate(e, t) { if (void 0 !== e) { const i = this.constructor._classProperties.get(e) || defaultPropertyDeclaration; return this._requestPropertyUpdate(e, t, i) } return this._invalidate() }
    _requestPropertyUpdate(e, t, i) { return this.constructor._valueHasChanged(this[e], t, i.hasChanged) ? (this._changedProperties.has(e) || this._changedProperties.set(e, t), !0 === i.reflect && (void 0 === this._reflectingProperties && (this._reflectingProperties = new Map), this._reflectingProperties.set(e, i)), this._invalidate()) : this.updateComplete }
    async _invalidate() {
    ...
    _validate() {
        if (this._instanceProperties && this._applyInstanceProperties(), this.shouldUpdate(this._changedProperties)) {
            ...
            this.update(e), this._markUpdated(), this._updateState & STATE_HAS_UPDATED || (this._updateState = this._updateState | STATE_HAS_UPDATED, this.firstUpdated(e)), this.updated(e)
...
UpdatingElement._attributeToPropertyMap = new Map, UpdatingElement._finalized = !0, UpdatingElement._classProperties = new Map, UpdatingElement.properties = {};
class LitElement extends UpdatingElement {
    update(e) {
        super.update(e);
        const t = this.render();
        t instanceof TemplateResult && this.constructor.render(t, this.renderRoot, { scopeName: this.localName, eventContext: this })
    }
    render() {}
}
LitElement.render = render$1;
const MEDIA_INFO = [{ attr: "media_title" }, { attr: "media_artist" }, { attr: "media_series_title" }, { attr: "media_season", prefix: "S" }, { attr: "media_episode", prefix: "E" }],
    ICON = { dropdown: "mdi:chevron-down", mute: { true: "mdi:volume-off", false: "mdi:volume-high" }, next: "mdi:skip-next", playing: { true: "mdi:pause", false: "mdi:play" }, power: "mdi:power", prev: "mdi:skip-previous", send: "mdi:send", shuffle: "mdi:shuffle-variant", volume_down: "mdi:volume-medium", volume_up: "mdi:volume-high" };
class MiniMediaPlayer extends LitElement {
    constructor() { super(), this._overflow = !1 }
    static get properties() { return { _hass: Object, config: Object, entity: Object, source: String, position: Number, active: Boolean, idle: Boolean, _overflow: Boolean } }
    set hass(e) {
        const t = e.states[this.config.entity];
        this._hass = e, t && this.entity !== t && (this.entity = t)
    }
    set overflow(e) { e !== this._overflow && (this._overflow = e) }
    setConfig(e) {
        if (!e.entity || "media_player" !== e.entity.split(".")[0]) throw new Error("Specify an entity from within the media_player domain.");
        const t = { artwork: "default", artwork_border: !1, background: !1, consider_idle_after: !1, consider_pause_idle: !1, group: !1, hide_controls: !1, hide_icon: !1, hide_info: !1, hide_media_info: !1, hide_mute: !1, hide_power: !1, hide_volume: !1, icon: !1, max_volume: 100, more_info: !0, power_color: !1, scroll_info: !1, short_info: !1, show_progress: !1, show_shuffle: !1, show_source: !1, show_tts: !1, title: "", toggle_power: !0, volume_stateless: !1, ...e };
        t.consider_idle_after = 60 * Number(t.consider_idle_after) || !1, t.max_volume = Number(t.max_volume) || 100, t.collapse = t.hide_controls || t.hide_volume, t.short_info = t.short_info || t.scroll_info || t.collapse, this.config = t
    }
    shouldUpdate(e) { if (this.entity && (e.has("entity") || e.has("source") || e.has("position") || e.has("_overflow"))) return this.active = this._isActive(), this.config.show_progress && this._checkProgress(), !0 }
    updated() { this.config.scroll_info && this._computeOverflow() }
    render({ _hass: e, config: t, entity: i } = this) { const r = this._computeArtwork(); return html `
      ${this._style()}
      ...
      </ha-card>`}_computeName(){return this.config.name||this.entity.attributes.friendly_name}_computeCover(e){return e&&"cover"==this.config.artwork?e:this.config.background}_computeArtwork(){return!(!this.entity.attributes.entity_picture||""==this.entity.attributes.entity_picture||"none"===this.config.artwork||!this.active)&&this.entity.attributes.entity_picture}_computeIcon(){return this.config.icon?this.config.icon:this.entity.attributes.icon||"mdi:cast"}_computeOverflow(){const e=this.shadowRoot.querySelector(".marquee");if(e){const t=e.clientWidth>e.parentNode.clientWidth;this.overflow=!(!t||!this.active)&&7.5+e.clientWidth/100}}_renderIcon(e){if(!this.config.hide_icon)return this.active&&e&&"default"==this.config.artwork?html`
    ...
    `}_renderPower(){return html`
      ...
      </paper-icon-button>`}_renderMediaInfo(){if(this.config.hide_media_info)return;const e=MEDIA_INFO.map(e=>({info:this._getAttribute(e.attr),prefix:e.prefix||"",...e})).filter(e=>""!==e.info);return html`
